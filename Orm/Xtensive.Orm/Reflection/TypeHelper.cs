          ...
          for (var currentSuffix = 0; currentSuffix < associateTypeSuffixes.Length; currentSuffix++) {
            ...
            if (currentForType.IsArray && genericArguments.Length == 1) {
              ...
              if (itemType.IsPrimitive || (itemType.IsValueType && !itemType.IsGenericType)) {
                ...
                associateTypeName = AddSuffix($"{location.Item2}.{nonGenericArrayName}", associateTypeSuffix);
                suffix = CorrectGenericSuffix(associateTypeName, 0);
                if (Activate(location.Item1, suffix, null, constructorParams) is T result1) {
                  foundForType = currentForType;
                  return result1;
                }
                ...
                associateTypeName = AddSuffix($"{location.Item2}.{nonGenericArrayName}", associateTypeSuffix);
                suffix = CorrectGenericSuffix(associateTypeName, 0);
                if (Activate(location.Item1, suffix, null, constructorParams) is T result2) {
                  foundForType = currentForType;
                  return result2;
                }
